...
class EKF:
    ...
    def update(self, q: np.ndarray, gyr: np.ndarray, ang: np.ndarray, acc: np.ndarray, ang_R: np.ndarray = None, mag: np.ndarray = None) -> np.ndarray:
        """
        ...
        if (a_norm>0) & (ang is None):
            ...
            v   = ang - q_t
            H   = np.identity(4)
            S   = H@P_t@H.T + np.identity(4)*0.0000001
